...
`area:*`는 실제로 건드린 코드나 문서 표면입니다. 리뷰 경로를 돕는 보조 라벨이라, 애매하면 생략해도 됩니다. 예를 들어 `track:product` 작업이라도 PR 템플릿 파일을 바꾸면 `area:process`나 `area:docs`를 붙일 수 있습니다.
