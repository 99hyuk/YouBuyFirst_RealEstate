...
| `type:dataset` | seed, fixture, 종목 마스터 데이터 | 종목 CSV, 테스트 HTML |
...
| `track:data` | `data` | 종목 인식, 감성 분류, 열기 지수, 30분 집계 |
...
`area:*`는 실제로 건드린 코드나 문서 표면입니다. 리뷰 경로를 돕는 보조 라벨이라, 애매하면 생략해도 됩니다. 예를 들어 `track:ops` 작업이라도 PR 템플릿 파일을 바꾸면 `area:process`나 `area:docs`를 붙일 수 있습니다.
