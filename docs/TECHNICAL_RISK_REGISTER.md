...
이 문서는 너나사 최종 기획을 구현하면서 생길 수 있는 기술, 제품, 운영 이슈를 미리 모아두는 목록입니다. 실제 장애가 발생한 뒤의 상세 복구 기록은 `docs/TROUBLESHOOTING_GUIDE.md`와 PR/Notion 작업 로그에 남기고, 이 문서에는 "무엇을 조심해야 하는지"와 "어떻게 방어할지"를 계속 추가합니다.
...
- 트랙: crawl/data/market/trade/agent/front/ops
- 상태: 후보/발생/완화/닫힘
...
- 방어: 공개 화면에는 제한된 quote snapshot만 직접 표시하고, 차트 전체는 우선 외부 위젯을 사용합니다. 직접 표시하는 시세에는 `30분 지연`, provider, `asOf`, `stale` 상태를 항상 붙입니다. 원시 분봉, 호가, 대량 OHLC, 다운로드/API 형태의 재배포는 별도 계약 전까지 하지 않습니다.
- 확인: 모든 market UI에 `asOf`, 지연 시간, 출처, stale 상태가 있는지 화면 QA로 확인합니다. provider 변경 전에는 공개 표출 가능 범위를 문서화합니다.
...
- 트랙: ops
- 상태: 후보
- 증상: PR은 머지됐는데 `CURRENT_HANDOFF`, `TASKS`, Notion 작업 로그가 다른 상태를 가리킵니다.
- 영향: 새 채팅과 병렬 에이전트가 오래된 전제를 따릅니다.
- 방어: PR 머지 후 ops가 주요 handoff와 task list를 확인합니다. 큰 구조 변경은 문서 PR로 먼저 남깁니다.
- 확인: PR 종료 시 `CURRENT_HANDOFF`, `TASKS`, 관련 트랙 README의 다음 작업이 맞는지 확인합니다.
...
- 확인: 큰 작업 후 input token 최대/평균을 비교하고 `DOCUMENTATION_GUIDE.md`의 예산을 지킵니다.
...
- 트랙: front/data/market/trade/agent
- 상태: 후보
- 증상: front mock 화면이 실제 backend/API가 제공하기 어려운 필드와 상태를 전제합니다.
- 영향: API 연동 때 대규모 재작업이 생깁니다.
- 방어: front-first discovery는 유지하되, mock field마다 소유 트랙과 계약 후보를 기록합니다. 실제 API 전환 전에는 fixture와 contract test를 맞춥니다.
- 확인: dashboard fixture 필드를 `data/market/trade/agent` contract 후보로 역추적합니다.
...
- 확인: `FINAL_PRODUCT_PLAN.md`와 front navigation에서 부동산이 주식 MVP의 필수 탭이나 데이터 계약으로 들어가지 않았는지 확인합니다.
...
- 방어: `docs/STOCK_DETAIL_COPY_GUIDE.md` 기준으로 최소 2개 이상의 근거 묶음이 같은 방향일 때만 `spicy`/`roast` 톤을 씁니다. 헤드라인 아래에는 기준일, 기간, 근거 지표 3-5개를 함께 표시합니다.
