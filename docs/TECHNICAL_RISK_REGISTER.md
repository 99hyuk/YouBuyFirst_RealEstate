...
- 트랙: market/front/ops
- 상태: 후보
- 증상: 지연 시세나 mock data를 `실시간`처럼 보여줍니다.
- 영향: 데이터 제공자 약관, 사용자 신뢰, 투자 자문 오해가 생깁니다.
- 방어: provider 정책을 확인하고 `mock`, `지연`, `stale`, `asOf`를 화면에 표시합니다. 외부 quote 사용 전에는 공개 재배포 가능 여부를 문서화합니다.
- 확인: 모든 market UI에 `asOf`와 지연/출처 상태가 있는지 화면 QA로 확인합니다.
