...
지역 분석 지도와 대시보드의 가격 흐름 대표 지표는 `docs/domains/realestate/OFFICIAL_PRICE_INDEX_DICTIONARY.md`를 정본으로 둡니다.

- 1순위 지도 지표는 metricCode `reb_weekly_apt_sale_price_index_region`과 `reb_monthly_apt_sale_price_index_region`입니다.
- DB 적재 dataset은 주간 `reb_rone_weekly_apt_sale_price_index_region`, 월간 `reb_rone_monthly_apt_sale_price_index`입니다.
- 공식명은 `전국주택가격동향조사_주간아파트동향_매매가격지수(지역별)`입니다.
- 가격지수는 `factType=price_index`로 지수 원값을 저장합니다. 지도용 `changePct`는 저장된 최신 지수와 기준 시점 지수를 비교해 snapshot 생성 시 산출합니다.
- 지도 색상 진하기는 같은 지표의 기준 시점 index 비교값에 비례해야 하며, 임의 seed 색상을 공식값처럼 쓰지 않습니다.
- `official_missing`, `ingestion_missing`, `mapping_unresolved`를 분리해 저장합니다. 화면에서 검게 보이는 지역은 공식 원천에 실제로 없는지 먼저 확인해야 합니다.
- 주간 운영 수집은 Spring Batch job, 월간 운영 수집은 R-ONE Open API 기반 pipeline 수집기로 구현합니다. R-ONE 지역 현황 지도 응답은 최신 월과 지역 코드 매핑 보조로만 사용합니다.
...
- `sido` region은 전국 흐름, 필터 scope, 상위 roll-up 요약에 사용합니다. 지역 반응 화면의 기본 TOP 행으로는 세우지 않습니다.
- 지역 언급량 랭킹의 기본 대상은 `sigungu`, `eupmyeondong`, `living_area`처럼 사용자가 실제로 비교할 수 있는 세부 지역입니다.
- 아파트 관심 후보의 기본 대상은 `complex` target입니다. 실제 아파트 단지, 또는 공공데이터/운영자 검증으로 만든 아파트 단지 target만 포함합니다.
- `type=complex` ranking이 비어 있을 때 region row로 대체하지 않습니다. 화면은 `수집 전`, `insufficient`, `단지 alias 미연결` 상태를 표시합니다.
- 특정 아파트 단지 언급은 원본 complex snapshot에 직접 저장하고, `contains` edge를 통해 상위 동·구·생활권·시도 region snapshot에 roll-up합니다. roll-up 관측치는 `matchSource=target_graph:contains`, `matchedText=rollup:{원본 targetId}`로 남겨 직접 지역 언급과 구분합니다.
- 지역 EvidenceLog는 직접 지역 언급, 하위 지역 roll-up, 단지 언급 roll-up을 함께 볼 수 있지만, 표현에서는 어떤 비중이 직접 언급이고 어떤 비중이 단지 파생인지 숨기지 않습니다.
- 커뮤니티에서 처음 발견한 단지 후보는 원칙적으로 `candidate` 상태로 두며 matcher 입력에 바로 섞지 않습니다. 다만 공개 게시글의 구조화 실거래 표에서 승인 지역 alias와 함께 추출된 후보는 `source=community:structured-trade-table`일 때만 allow-list로 reaction snapshot에 사용할 수 있습니다.
- `rejected` target, alias, edge는 과거 snapshot이 남아 있어도 ranking 조회와 최신 window 후보 조회에서 제외합니다.
...
`GET /internal/realestate/regions?regionLevel=&limit=&page=`는 pipeline이 지역 계층을 page 단위로 export하는 내부 API입니다. 단지 registry refresh는 이 응답의 `eupmyeondong` target을 읽어 실거래/전월세 fact가 5자리 시군구 `legalDongCode`만 갖고 있더라도 `시군구코드 + 법정동명` 조합으로 대표 `regionTargetId`를 동 단위까지 우선 연결합니다.

### `user_watch_targets`

`user_watch_targets`는 로그인 사용자가 직접 저장한 관심 대상입니다. 공개 반응 순위나 시장 지표가 아니라 개인 저장 목록이며, `app_users.id`와 연결합니다.

| 필드 | 타입 후보 | 기준 |
| --- | --- | --- |
| `id` | uuid/string | 관심 저장 row id |
| `userId` | uuid/string | `app_users.id` |
| `targetType` | enum | `region`, `complex`만 허용 |
| `targetId` | string | 지역 리포트 target id 또는 실거래 상세의 선택 item id |
| `displayName` | string | 오른쪽 `관심` 탭에 보여줄 지역명/단지명 |
| `landingPath` | string | `/realestate/`로 시작하는 복귀 경로 |
| `createdAt` / `updatedAt` | datetime | 저장/갱신 시각 |

구현 계약:

- `(userId, targetType, targetId)`는 unique입니다. 같은 대상을 다시 저장하면 `displayName`, `landingPath`, `updatedAt`을 갱신합니다.
- `GET /api/realestate/watch-targets`는 현재 로그인 사용자의 관심 목록을 최신 갱신순으로 반환합니다.
- `POST /api/realestate/watch-targets`는 `{ targetType, targetId, displayName, landingPath }`를 저장합니다.
- `DELETE /api/realestate/watch-targets?targetType=&targetId=`는 현재 로그인 사용자의 해당 관심만 해제합니다.
- 비로그인 요청은 401로 처리하며, 다른 사용자의 관심 row를 조회하거나 삭제할 수 없습니다.
...
- `GET /internal/realestate/target-edges?targetId=&direction=&edgeType=&reviewState=&limit=&page=`는 운영/파이프라인 확인용 edge 목록입니다. pipeline client는 한 페이지가 꽉 차면 다음 `page`를 반복 조회해 1000건을 넘는 단지→지역 `contains` edge도 roll-up 입력에서 누락하지 않습니다.
...
- 같은 게시글에서 단지 alias와 동/구 alias가 함께 매칭되거나 여러 하위 target이 같은 상위 target으로 roll-up되더라도, 상위 target snapshot의 mention은 `target + externalId + source` 기준 1건으로만 집계합니다. 중복 후보가 있으면 confidence가 더 높은 관측치를 대표 roll-up으로 남깁니다.
...
- 단지 registry는 실거래/전월세 fact의 공식 단지명으로 `official`, `법정동명 + 공식 단지명`으로 `nearby_area`, 3개 이상 한글 토큰의 초성 축약으로 `short_name`, `법정동명 + short_name`으로 `nearby_short_name` alias를 만듭니다. `마포 래미안 푸르지오`처럼 앞 토큰이 지역 접두어로 보이는 경우에는 전역 `래미안 푸르지오` alias를 바로 승인하지 않고 `아현동 래미안 푸르지오` 같은 `nearby_core_name`만 생성해 단지 매칭 범위를 지역 문맥 안으로 제한합니다. 공공데이터 단지명이 `마포래미안푸르지오`처럼 붙어서 들어오면 알려진 브랜드 토큰을 보수적으로 분리해 `마래푸`, `아현동 마래푸` alias를 생성합니다.
- 단지 registry의 대표 지역은 `legalDongCode` 정확 매칭, `시군구코드 + 법정동명` 매칭, 5자리 시군구 fallback 순서로 선택합니다. 따라서 `legalDongCode=11440`, `legalDongName=아현동` fact와 backend `eupmyeondong` region export가 함께 있으면 단지는 `region-seoul-mapo`가 아니라 `region-1144010100` 같은 동 target에 우선 연결됩니다.
- 실거래/전월세 fact가 아직 없는 단지도 커뮤니티 게시글에서 실제 단지명이 관측되면 `community_observed` complex target으로 등록할 수 있습니다. 이때 `source=community:observed-complex-seed`, `aliasType=community_observed|community_alias`, `markerDataStatus=community_observed`, `markerStale=true`로 저장하고, 좌표·실거래가·전세가·공급 fact는 `unknown`/`확인 필요` 상태를 유지합니다. 이 경로는 시연/운영 후보를 살리기 위한 보조 경로이며, 공식 provider 검증이 끝난 단지처럼 표현하지 않습니다.
...
- `GET /internal/realestate/aliases?reviewState=approved&ambiguous=false&targetType=region|complex&limit=&page=`은 pipeline matcher JSONL export 후보입니다. pipeline client는 page를 반복 조회해 동·구급 지역 alias와 실제 아파트 단지 alias가 대량으로 늘어나도 첫 페이지만 매칭하는 상태로 떨어지지 않습니다.
...
`content_items`는 뉴스, 리포트, 영상, 블로그/커뮤니티 링크처럼 외부 원문으로 보내야 하는 콘텐츠 카드의 정본입니다. 원문 전문을 복제하지 않고 제목, 제한 snippet, URL, 발행/수집 시각, 상태 라벨만 저장합니다. 뉴스룸 전역 feed 항목은 지역/단지와 무관하므로 `targets: []`로 저장할 수 있습니다. `content_target_links`는 콘텐츠와 지역/단지 target의 다대다 연결이 필요할 때만 사용합니다. `reviewState=approved`인 연결만 target 상세 content 목록과 timeline에 노출합니다.
...
`newsroomContentRefreshJob`은 프론트 `/newsroom`용 전역 feed 적재 Job입니다. RSS/XML 또는 후속 검색/API adapter에서 제목, 제한 snippet, URL, domain, 발행 시각만 추출해 `content_items`에 upsert하고 target link는 만들지 않습니다. 검색 순위나 노출량은 관심도 지표로 쓰지 않습니다. 저장 성공 후에는 SSE `topic=newsroom` 업데이트를 발행해 열린 프론트 화면이 뉴스룸 API를 재조회하게 합니다.
...
리포트 본문은 기대 지점, 우려 지점, 종합 리포트 body, 근거 적재소 source ledger로 나눕니다. 매수·매도·청약 행동을 권유하지 않고, 실거래·전세·공급·정책·뉴스/리포트 근거를 함께 보는 관찰형 판단으로만 씁니다. 기대/우려는 1-2개를 기본으로 하며 의미 없는 항목을 억지로 채우지 않습니다. 각 기대/우려 포인트는 `전세·실거래 동행`, `입주 물량 부담`처럼 짧은 명사형으로 끝내고 서술형 문장이나 마침표를 쓰지 않습니다. 종합 리포트 body는 운영 설명이나 자동화 설명이 아니라 금융권 리포트처럼 냉정한 문체로 쓰며, `평가:`에는 해당 지역의 수요·공급·전세·정책·산업 변수를 해석하고 `전망:`에는 `관망 우위`, `선별 관망`, `선별 회복 가능성`, `방어력 우위`처럼 가격 단정이 아닌 방향성 판단을 드러냅니다. 시군구 리포트는 상위 시도 문장을 재사용하지 않고, 같은 시도 안에서도 생활권·산업·전세·공급 리스크가 다른 경우 기대/우려 포인트와 body를 분리합니다. 확인되지 않은 최근 이슈 후보는 source row의 `dataStatus=candidate` 또는 관련 content link의 `reviewState=candidate`로 남기고, 결론처럼 표현하지 않습니다.
...
| `summary` | 짧은 요약 |
| `body` | 금융권 리포트형 종합 본문. `평가:`와 `전망:`을 포함하되 가격 단정/행동 CTA 금지 |
...
| 필드 | 기준 |
| --- | --- |
| `id` | 안정 source id. 예: `reb-r-one`, `applyhome` |
| `title` | 화면에 표시할 공식 출처명 |
...
| `status` | `확인 완료`, `확인 필요` 등 |

`market_data_schedules` 필드:

| 필드 | 기준 |
| --- | --- |
| `id` | source와 날짜를 포함한 안정 일정 id |
...
| `status`, `dataStatus`, `stale` | `공표 확인/published`, `공식 일정/scheduled`, `출처 확인 필요/source_error`를 구분 |
| `lastCheckedAt` | 배치 확인 시각 |
| `asOf` | 일정 row 기준일 |
...
`daily_briefings`는 대시보드 3줄 요약과 상세 브리핑 본문이 공유하는 저장 결과입니다. 화면은 저장된 최신 브리핑을 읽고, 배치가 새 결과를 발행하기 전에는 대시보드가 mock 문장을 조합하지 않습니다.

필드 기준:

| 필드 | 기준 |
| --- | --- |
| `briefingId` | `daily-briefing-YYYYMMDD-vN` 형식의 upsert key |
| `briefingDate` | 브리핑 기준일 |
| `title` | 상세 화면 제목 |
| `summaryHeadlines[]` | 대시보드에 그대로 표시하는 짧은 명사형 헤드라인. 정확히 3개 |
| `sections[]` | `오늘의 핵심 흐름`, `주목할 지역과 이유`, `시장 변수`, `관련 뉴스·리포트` 본문 섹션 |
| `focusRegions[]` | 주목 지역 보조 표시용 배열. `targetId`, `label`, `reason`, `displayOrder` 권장 |
| `sourceItems[]` | 관련 뉴스·리포트 링크 후보. 관심도 점수나 결론 근거로 단독 승격하지 않음 |
| `modelName` | 예: `claude opus 4 8`. 룰 fallback이면 `rule-based` |
| `promptVersion` | 예: `daily-briefing-v1` |
| `generatedAt` | 배치 생성 시각 |

구현된 백엔드 계약:

- `POST /internal/realestate/daily-briefings`는 `briefings[]`를 받아 `briefingId` 기준으로 브리핑, 본문 섹션, source item을 upsert합니다.
- `GET /api/realestate/daily-briefings/latest`는 최신 `generatedAt desc` 브리핑 1건을 반환합니다. 저장 결과가 없으면 `204 No Content`입니다.
- 저장 단계에서 `summaryHeadlines`가 3개가 아니거나, 본문 섹션 제목이 누락되거나, 화면용 시간 구간 제목/행동 권유 문구가 들어가면 `400`으로 거부합니다.

pipeline Daily Briefing 조립:

- `realestate-daily-refresh --enable-realestate-daily-briefing-refresh`는 market fact, map layer, newsroom, 수동 curated pack을 input pack으로 묶고 `POST /internal/realestate/daily-briefings`로 발행합니다.
- `--realestate-daily-briefing-use-gms-llm`을 켜면 GMS OpenAI-compatible chat endpoint를 사용합니다. 기본 모델은 전체 시장 브리핑 품질을 위해 `claude opus 4 8`입니다.
- LLM 호출이 실패하거나 응답 shape가 맞지 않으면 배치는 룰 기반 fallback 브리핑을 발행하고 step 상태를 `PARTIAL`로 남깁니다.
- input pack과 프롬프트는 `오늘`, `최근`, `중기 흐름`을 사용자-facing 섹션으로 나누지 않고 내부 렌즈로만 사용합니다. 상세 화면도 `24시간`, `7일`, `1개월`, `3개월` 같은 구간 제목을 표시하지 않습니다.

## API 후보

```text
GET /api/realestate/targets/search?q=&mode=&limit=
...
GET /api/realestate/daily-briefings/latest
GET /api/realestate/indicators?legalDongCode=&period=
GET /api/realestate/market-data-schedules?month=
...
GET /internal/realestate/market-data-targets?enabled=&limit=&page=
GET /internal/realestate/aliases?reviewState=&ambiguous=&targetType=&limit=&page=
GET /internal/realestate/target-edges?targetId=&direction=&edgeType=&reviewState=&limit=&page=
...
`GET /api/realestate/targets/search`는 지역/단지/생활권 공통 target 검색 API입니다. 대시보드 자동완성은 `mode=autocomplete&limit=8`을 사용하며, backend는 자동완성 모드에서 2글자 미만 검색어를 빈 결과로 처리하고 limit을 최대 8개로 제한합니다. 조회는 target name/slug/id/legalDongCode와 approved non-ambiguous alias를 함께 보되, 먼저 정확 일치와 앞부분 일치를 우선 정렬하고 부분 일치는 후순위 fallback으로 둡니다. 자동완성에서는 `대전 갈마동`처럼 상위 지역명과 동명이 함께 입력될 때 마지막 토큰으로 한 번 더 보조 조회하고, region 타입에 한해서 저장명 내부 포함 검색을 허용해 `eupmyeondong` row를 실거래 필터 후보로 반환할 수 있어야 합니다. 응답 row는 `targetId`, `targetType`, `displayName`, `slug`, `reviewState`, `dataStatus`, `regionLevel`, `parentTargetId`, `legalDongCode`, `regionCode`를 포함합니다. 단지 row의 자체 `legalDongCode`가 비어 있으면 부모 지역의 `legalDongCode`/`regionCode`를 내려 대시보드에서 실거래 화면의 지역 필터를 함께 복원할 수 있어야 합니다.
...
`GET /api/realestate/reactions/rankings`는 `real_estate_reaction_snapshots`를 지역/단지 랭킹 화면용으로 반환합니다. `windowStart`가 없으면 해당 target type의 최신 window를 반환하고, 없으면 빈 `items`와 `coverageStatus=empty`를 반환합니다. `parentTargetId`가 없는 기본 `type=region` 랭킹은 `sido` roll-up을 제외하고 `sigungu`, `eupmyeondong`, `living_area` 같은 세부 지역 target만 반환합니다. `parentTargetId`를 받으면 parent 지역은 범위 조건으로만 쓰고, `type=region` 랭킹 행에서는 parent 자체를 제외한 하위 지역 target만 반환합니다. `type=complex`에서는 parent와 모든 하위 지역 scope 안에 `real_estate_complexes.region_target_id`가 있는 실제 아파트 단지도 포함합니다.

`realestate-top10-readiness` pipeline 명령은 운영/시연 전 검증용 읽기 전용 진단입니다. backend의 approved complex alias, `contains` edge, `type=complex` ranking, `type=region` ranking을 조회해 `complexRegistry`, `reactionSnapshots`, `frontTop10` 상태를 반환합니다. `status=READY`는 실거래/전월세 기반 complex alias/edge가 있고, complex ranking이 실제 `complex` 행만 포함하며, region ranking에 광역 `sido` row가 섞이지 않았다는 뜻입니다.
...
`GET /api/realestate/market-facts`는 운영/검증용 범용 market fact 목록입니다. `targetId`, `legalDongCode`, `factType`, `limit`, `page`로 좁힐 수 있으며, 특정 지역/단지 상세 화면은 가능한 한 `GET /api/realestate/targets/{targetId}/market-facts`를 우선 사용합니다. 단지 registry 배치는 promoted `apt_trade`/`apt_rent` fact를 이 API의 `page` 단위로 반복 조회해 첫 페이지 이후 실거래/전월세 fact도 complex target 후보로 반영합니다.

`GET /internal/realestate/market-data-targets`는 공공데이터 수집 대상과 법정동 코드, 내부 region target 매핑을 pipeline에 내보내는 내부 API입니다. `enabled`, `limit`, `page`를 지원하며, 단지 registry refresh는 이 API를 page 단위로 반복 조회해 실거래/전월세 fact에서 만든 complex target을 부모 시군구·동 region에 연결합니다.
