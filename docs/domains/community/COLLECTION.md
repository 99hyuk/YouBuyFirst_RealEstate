...
소스별 활성화 상태의 의미와 트랙별 책임은 `docs/archive/superpowers/items/specs/2026-05-15-source-activation-state-design.md`를 기준으로 봅니다. persistent `CrawlTarget` queue와 DB 기반 backoff 상태 설계는 `docs/archive/superpowers/items/specs/2026-05-15-crawl-target-queue-design.md`를 기준으로 봅니다. source policy는 `CrawlTarget`보다 상위 게이트이고, `CrawlTarget`은 허용된 소스 안에서 어느 게시판/종목을 언제 다시 수집할지 정하는 실행 단위입니다.
