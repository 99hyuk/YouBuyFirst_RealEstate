...
- `quickPanel`: 관심/최근 본 미리보기. 관심은 로그인 세션 기반 `user_watch_targets` 목록을 사용하고, 최근 본은 front 로컬 상태로 표시한다. 사이트 첫 진입에서는 채팅 패널이 기본으로 열려 `GET /api/chat/messages?limit=100`, `/api/chat/messages/stream`, `/api/chat/presence`를 바로 연결하고 최신 메시지가 보이는 하단 위치에서 시작한다. 로그인 사용자와 게스트 닉네임 입력은 `POST /api/chat/messages`로 저장한다. 게스트 메시지는 `session_id`로 같은 브라우저 세션의 내 메시지를 구분하고 인증마크 없이 표시한다. 게스트 닉네임은 저장 전 `GET /api/chat/nickname-availability`로 가입자 닉네임 충돌을 확인하고, 서버 POST도 같은 충돌을 거절한다. 열린 채팅창은 `/api/chat/messages/stream` SSE로 새 메시지를 즉시 추가하며, 주기 재조회는 stream 장애 fallback으로 둔다. 채팅 메시지는 선택적으로 `attachment`를 포함하며, `region`/`complex` 타입, 대상 id, 제목, 보조 설명, 기간 라벨, 등락률 표시값, 내부 복귀 경로를 저장한다. 첨부 카드는 `종류+제목 / 필요할 때만 보이는 설명 / 기간+등락률`로 렌더링한다. 실거래는 지역·동·가격 설명을 유지해 3줄, 지역 분석은 기간 설명이 중복되면 숨겨 2줄로 보인다
...
- 기간 선택은 지도 제목 옆의 compact rectangular segmented control로 표시한다. 별도 `기간` 라벨 버튼은 두지 않고, 5개 기간 옵션은 같은 glass track 안에 둔다. 선택된 기간은 track 안에서 움직이는 밝은 amber slider thumb과 얇은 하단 라인으로 구분한다. 데스크톱에서는 `전국 흐름 지도` 라벨과 `전국 부동산 흐름` 제목 묶음의 전체 높이를 기준으로 세로 중앙에 둔다. 모바일에서는 제목 아래로 내려가며 한 줄 안에서 viewport 폭에 맞춘다.
...
- 지역 버튼: 각 시도 중심점에 표시
- 하단 요약: 가장 높은 상승, 가장 많은 하락, 지역 단위. `한국부동산원 R-ONE · 공공데이터 반영 · 최신 반영 · 기준 시각` 같은 긴 출처/상태 문자열은 하단 요약에 노출하지 않는다.
...
- 하단 요약: 가장 높은 상승, 가장 많은 하락, 하위지역. `다음 단계` 카드는 두지 않고, 지역 단위 카드는 하위지역 개수와 `하위지역 기준`만 표시한다.
- 선택 리포트 패널
  - 지역 리포트 헤더
  ...
  - 기대 지점/우려 지점: 대표 브리핑 카드 본문 전체 폭을 써서 각각 1-2개만 보여준다. 의미 있는 판단 근거가 없으면 억지로 3개를 채우지 않는다. 행동 권유가 아니라 공식 지표, 전세, 실거래, 공급, 정책 일정 기준의 관찰 포인트로만 쓴다. 오른쪽 패널에서 읽기 편하도록 본문 글씨는 작은 보조 텍스트처럼 보이지 않게 한다.
  ...
  - 시각 구조: `/realestate/daily-briefing` 상세 화면의 narrative panel과 관련 근거 ledger 리듬을 참고하되 동일 컴포넌트처럼 보이지 않게 한다. 처음 보이는 오른쪽 패널은 `AI 핵심 브리핑/기대·우려`, `AI 분석 리포트/선택 기간 등락률`, `관련 뉴스·리포트`만 보여준다. 대표 브리핑 카드는 라벨만 두고, 리포트 headline 문장은 이 영역에 반복하지 않는다. 아래 본문 전체 폭에 기대 지점과 우려 지점을 나란히 배치한다. 등락률은 분석 리포트 본문 첫머리의 큰 수치 박스로 두며, 지역 내 비교/공급·정책·보조 반응/단지 후보는 후속 상세 후보로 둔다.
...
- 리포트 본문, 기대 지점, 우려 지점, 근거 적재소는 `GET /api/realestate/targets/{targetId}/regional-report`의 최신 저장본을 우선 사용한다. 지도 기간 탭은 색상과 등락률 숫자만 바꾸며 저장 리포트 본문을 기간별로 다시 쓰지 않는다.
- 저장 리포트가 없으면 지도 layer, target market facts, timeline/content, 최신 EvidenceLog를 조합한 fallback을 보여준다. 이때도 값이 없으면 `공식 데이터 없음`, `수집 전`, `상위 지역만 반영`, `공개 지연 가능` 상태로 둔다.
- 화면 요청 중 AI를 호출하지 않는다. AI API 자동화는 backend/pipeline이 `POST /internal/realestate/regional-reports`로 같은 target의 최신 리포트를 교체하는 방식으로 붙인다.
...
- `real_estate_regional_reports`, `real_estate_regional_report_sources`: 지도 기간과 무관하게 target별 최신 리포트 1개와 근거 ledger
