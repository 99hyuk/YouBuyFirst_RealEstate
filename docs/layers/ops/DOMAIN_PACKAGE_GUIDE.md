...
| 영역 | 목표 이름 | 이유 |
| --- | --- | --- |
| 대상 정본 | `region`, `complex`, `realestate_target` | 부동산 식별자는 지역 법정동 코드와 단지 id 중심입니다. |
| 시장 사실 snapshot | `market fact snapshot` | 실거래, 전세, 매물, 청약, 정책 상태를 provider/asOf/stale와 함께 기록합니다. |
| 시장 시계열 | `market time series` | 일/주/월 단위 가격, 전세, 매물 흐름을 관찰합니다. |
| 반응 분석 | `analysis` | 감성 점수보다 반응 방향과 쟁점 판단을 다룹니다. |
| `metrics` | `indicator` | window 단위 집계와 반응 지표 snapshot을 소유합니다. |
| 지역/단지 평가 | `evidence-based area evaluation` | 매매 판단이 아니라 지역/단지에 대한 근거 있는 평가를 남깁니다. |
...
현재 active runtime 기준 패키지는 부동산 전용 모델을 우선합니다. 부동산에 맞지 않는 코드는 repo 안에 보존하지 않고 git history에서 확인합니다.

실제 정리는 기능 단위로 나눕니다.

1. 문서와 작업 영역을 부동산 중심으로 고정합니다.
2. `realestate` 도메인 문서와 패키지 skeleton을 만듭니다.
3. 공통 수집, 제한 원문 저장, 지표 snapshot 구조만 부동산 이름으로 유지합니다.
4. 화면, pipeline CLI, backend context에 부동산과 맞지 않는 route/entity/fixture/seed가 재유입되지 않게 테스트와 검색으로 확인합니다.

DB table 이름은 부동산 정본 스키마를 우선합니다. 기존 migration 안정성을 위해 package/class 이름 변경과 DB schema 변경은 작은 단위로 분리합니다.
