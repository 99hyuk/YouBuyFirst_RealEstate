...
4. 규칙 전파 대상은 우선 ui/community/realestate/indicator/agent 같은 장기 작업 브랜치입니다. 이미 머지되었거나 폐기 예정인 ops 보조 브랜치는 정리 후보로 둡니다.
...
- 코드/API/DB/배치/crawler/market fact/front 동작 PR과 정책 문서 PR은 Codex 리뷰를 요청하거나 자동 리뷰 결과를 확인합니다. PR 생성/ready 직후 리뷰가 아직 없으면 바로 merge/완료 처리하지 말고 review 상태로 둔 뒤 재확인합니다.
