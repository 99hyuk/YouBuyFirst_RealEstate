...
이 문서는 복제된 YouBuyFirst를 부동산 전용 프로젝트로 전환할 때 기준이 되는 제품 방향입니다. 기존 주식 프로젝트의 커뮤니티 반응 분석 구조는 재사용하지만, 부동산을 주식 도메인 안에 추가 기능처럼 넣지 않습니다.

## 한 줄 설명

너나사 부동산은 지역과 단지에 대한 실제 사람들의 반응, 뉴스/컬럼 이슈, 실거래/전세/매물 같은 시장 사실 데이터를 함께 보여주는 관찰형 분석 서비스입니다.
...
- 수집, 반응 분석, 지표화, 유사 과거 비교, 에이전트 근거 로그 흐름은 기존 주식 프로젝트와 거의 같습니다.
- 주식은 종목 universe가 비교적 한정되어 있지만, 부동산은 지역, 단지, 생활권, 정책 영향권으로 훨씬 잘게 나뉩니다.
- 부동산은 지역 단위 상승, 정책, 교통, 공급, 대출, 청약, 학군, 재건축 같은 이벤트에 민감합니다.
- 따라서 단순히 `stock`을 `realestate target`으로 바꾸지 않고 target graph, alias DB, policy timeline, source registry를 1차 구현 범위로 둡니다.

## 반드시 분리할 구조

- 주식의 `stock`, `instrument`, `symbol`, `quote`, `chart candle`
- 부동산의 `region`, `complex`, `transaction`, `rent`, `listing`, `policy event`
- 주식의 모의 주문/체결/포트폴리오와 부동산의 관찰형 분석
- 주식 시세 freshness와 부동산 실거래/매물/전세 데이터 freshness
...
5. 기존 주식 코드는 참고/비활성 영역으로 분류하고, 삭제는 별도 PR로 분리합니다.
