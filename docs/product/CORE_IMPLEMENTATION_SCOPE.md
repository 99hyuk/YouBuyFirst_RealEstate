...
사람들이 커뮤니티, 댓글, 뉴스, 컬럼에서 남긴 부동산 반응을 지역과 단지 기준 지표로 만들고, 실거래/전세/매물/정책 흐름 및 비슷한 과거 상황과 함께 보여준다.

## 주식 프로젝트와 같은 점

기술 흐름은 기존 주식 프로젝트와 거의 같습니다.
...
7. agent가 지표, 시장 사실, 유사 과거, 근거를 읽고 사용자용 평가 로그를 남깁니다.
8. dashboard와 target detail API로 노출합니다.

## 부동산이라 달라지는 점

### 1. 대상이 한정된 종목 목록이 아니라 쪼개진 지역/단지 우주다

주식은 종목 universe가 비교적 명확합니다. 부동산은 시/구/동, 생활권, 역세권, 학군, 재건축 구역, 단지처럼 대상이 계층적이고 계속 쪼개집니다.
...
### 4. 시장 사실은 시세 tick이 아니라 지연과 기준시각이 있는 데이터다

주식 quote처럼 실시간 가격으로 다루면 안 됩니다. 실거래, 전세, 매물, 공공데이터, 지수는 각각 지연과 기준시각이 다릅니다.
...
| 에이전트 판단 | 지표, 시장 사실, 과거 비교 | 지표 입력 -> AI 근거 검색 -> 맥락 종합 -> 근거 로그 | 지역/단지 평가와 caveat |
