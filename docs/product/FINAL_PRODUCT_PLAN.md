...
차트와 재무제표만 보여주는 도구가 아니라, 커뮤니티가 어떤 종목에 몰리고 어떤 이슈에 반응하는지 수치화합니다. 사용자는 종목별 커뮤니티 반응, 가격 변화, 통합 지표 기반 모의 성과 검증, AI 에이전트의 모의 판단을 함께 보면서 투자 아이디어를 점검합니다.
...
- 종목/window 단위 화면 결론은 `종합 커뮤니티 반응`으로 둡니다. 결론은 `낙관 우세`, `비관 우세`, `혼조`, `관망`처럼 하나로 보여주고, 그 아래에 `주요 쟁점` 비율을 붙여 왜 그런 결론이 나왔는지 설명합니다.
...
- 구체적인 버튼, 카드, 차트 문구는 `layers/ui` 화면 정보 구조가 나올 때 별도로 정합니다.
- 세부 기준은 `docs/domains/community/REACTION_GUIDE.md`를 따릅니다.
...
기사와 공시는 원문 전문을 재게시하지 않고 제목, 출처, 시간, 링크, 관련 키워드 중심으로 보여줍니다. 종목 상세의 팩트폭격 헤드라인은 커뮤니티 반응 요약이 아니라 시황, 기술 지표, 재무, 뉴스/공시, 컨센서스 기반의 종목 상태 요약입니다. 화면 문구는 투자 판단을 유도하지 않고 "관찰된 변화"와 "확인할 이벤트"로 제한하되, 제품 톤은 `docs/domains/agent/STOCK_DETAIL_HEADLINE.md`의 짠맛/팩트폭격 기준을 따릅니다.
...
최종 지표는 단순 댓글 수나 낙관/비관 비율이 아니라 언급량, 반응 강도, 증가율, 신뢰도, 소스 가중치, 의미적으로 묶인 쟁점 구조를 함께 반영합니다. 낙관/비관/중립 count는 초기 집계와 품질 점검을 위한 하위 signal일 뿐이며, 그 자체를 `개미 심리 지수`의 최종 신뢰 근거로 삼지 않습니다. 사용자 화면은 먼저 종목/window 단위 `종합 커뮤니티 반응` 하나를 보여주고, 이어서 그 결론을 만든 `주요 쟁점` 비율을 보여줍니다.

`개미 심리 지수`는 커뮤니티 반응 모멘텀의 대표 지표입니다. 종목별로 0-100 점수를 만들고, 구성 요소는 언급량 변화, 낙관/비관 반응 비율, 표현 강도, 인기글/개념글 확산, 소스 다양성, 표본 수 신뢰도, 시세 지연 상태를 함께 봅니다. 고도화 단계에서는 임베딩/클러스터링으로 만든 `issueMix`의 쟁점 비율, 쟁점별 확산도, 대표 근거 글, 과거 유사 window와의 비교를 지표 신뢰도에 반영합니다. 70 이상은 과열/낙관 우세, 30 이하는 공포/비관 우세, 45-55는 방향성 약함으로 표시하되, 실제 화면에서는 점수와 함께 표본 수와 신뢰도 배지를 항상 붙입니다.

임베딩/클러스터링은 화면 출시의 첫 단계는 아니지만, 궁극적으로 지표 신뢰도를 끌어올리는 핵심 분석 레이어입니다. 기본 흐름은 수집, 종목 인식, 반응 방향 분류, 30분 집계를 먼저 안정화하는 것이고, 이후 임베딩으로 의미가 비슷한 글을 묶어 `주요 쟁점`을 만들거나 과거의 유사 반응 상황을 찾습니다. 이 기능은 "종목을 찾는 정본"이 아니라 "종합 반응이 왜 그렇게 나왔는지 설명하고, 단순 count보다 더 신뢰할 수 있는 쟁점 기반 지표를 만드는 레이어"로 사용합니다.

벡터DB는 초기 MVP의 필수 저장소가 아니라, 임베딩을 장기적으로 검색 가능한 자산으로 만들 때 도입합니다. 도입 시에는 현재 window와 의미가 비슷한 과거 커뮤니티 상황, 유사 토픽을 가진 다른 종목, 같은 이슈가 여러 커뮤니티로 퍼진 흔적, 질문형 AI 분석에 필요한 근거 글을 빠르게 찾는 역할을 맡습니다.
...
- 열기 지수 순위
- 개미 심리 지수 순위와 급변 종목
- 종목별 종합 커뮤니티 반응과 주요 쟁점 비율
- 쟁점별 대표 근거 글과 요약
- 과거 유사 커뮤니티 반응 상황과 이후 가격/언급량 변화
- 비슷한 커뮤니티 토픽을 보인 다른 종목/과거 이벤트
- 1시간/24시간 급등 종목
- 반응 일관성, 표본 신뢰도, 소스 편중 주의 배지
- 과열, 공포, 무관심 구간 표시
- 통합 지표에 영향을 준 확산 글과 관련 종목 연결
...
커뮤니티 반응 데이터가 실제 가격 변화와 어떤 관계가 있었는지 검증합니다. 이 기능은 실거래 지시를 하지 않고, 통합 지표와 동일한 전략 rule을 과거 데이터에 적용해 사후 수익률을 비교합니다. source별 slice는 내부 검증용으로만 두며, 사용자 화면에서는 특정 커뮤니티 이름별 우열이나 특정 커뮤니티 전용 전략을 노출하지 않습니다.

초기 전략 후보:

- 개미 심리 지수 급등 종목 추종 실험
- 과열과 부정 반응이 동시에 증가한 종목 역추종 실험
- `issueMix`에서 실적 기대 비중이 높고 반응 일관성이 높은 종목 관찰 실험
- 언급량은 급증했지만 가격 반응이 약한 종목 관찰 실험
- 반응 일관성은 낮지만 확산 강도가 높은 종목 리스크 관찰 실험

비교 지표:

- 반응 포착 후 1시간, 6시간, 24시간, 3일, 7일 수익률
- 추종 전략과 역추종 전략의 성과 차이
- 전략별 적중률, 평균 수익률, 최대 낙폭
- 시장 전체 상승장/하락장 대비 초과 성과
- coverage 충족 여부와 표본 부족 구간 제외 여부

사용자 화면에서는 "어느 커뮤니티가 늘 맞는다"가 아니라 "이 통합 지표 조건은 과거에 이런 흐름으로 이어졌다"는 실험 결과로 표현합니다. 상세 기준은 `docs/product/COMMUNITY_SIGNAL_STRATEGY.md`를 따릅니다.

### 6. 모의투자와 거래 원장

사용자와 AI 에이전트가 동일한 가상 규칙에서 paper trading을 진행합니다. 주문은 실제 증권사 주문이나 결제가 아니라 내부 체결 엔진에서 snapshot 가격으로 가상 체결됩니다.

이 기능의 핵심은 실사용자 주문 동시성보다 거래성 도메인의 정합성입니다. 에이전트는 30분 집계나 가격 snapshot 같은 통계 윈도우를 보고 판단하며, 같은 윈도우와 전략 버전으로 중복 판단/중복 주문이 생기지 않도록 관리합니다. `simulation` 도메인은 주문, 체결, 정산, 원장, 포지션, 손익 갱신을 트랜잭션 경계 안에서 다루고, 리더보드는 원장과 가격 snapshot으로 재계산 가능해야 합니다.
...
수집 행위 자체가 약관, robots 정책, 데이터베이스 제작자 권리, 개인정보 이슈와 충돌할 수 있으므로, 관련 사례는 `docs/governance/LEGAL_RISK_CASES.md`에 별도로 기록합니다.
...
- StockReactionWindow: 종목/window 단위 종합 커뮤니티 반응, confidence, mention count, issueMix
- ForwardReturn: 신호 이후 가격 변화와 기간별 수익률
- BacktestRun: 모의 전략 rule, 실행 기간, universe, data quality
- StrategyPerformanceSnapshot: 통합 지표 기반 모의 전략 성과와 리스크 지표
...
- StockReactionWindow 도메인
- ForwardReturn 계산
- 통합 지표 기반 추종/역추종 모의 전략
- 나만의 모의 전략 rule JSON과 백테스트
- 시장 대비 성과 리포트
