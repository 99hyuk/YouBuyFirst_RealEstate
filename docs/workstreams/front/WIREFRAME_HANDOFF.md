...
이 문서는 프론트 와이어프레임 채팅이 끊겼을 때 복구하기 위한 짧은 정본입니다. 상세 plan/spec archive는 `docs/superpowers/`에 남아 있지만, 현재 작업은 이 handoff를 우선합니다. 과거 근거가 꼭 필요할 때만 파일 1개와 키워드 1개로 좁혀 봅니다.
