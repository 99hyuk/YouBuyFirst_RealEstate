# 너나사 (YouBuyFirst) MVP: 커뮤니티 크롤링/감성 수집 파이프라인

## Summary

MVP는 네이버 종토방과 에펨코리아에서 신규 글 묶음을 30분 주기로 수집하고, 국내 종목과 미국 상장 주식/ETF 언급을 인식해 종목별 언급량과 bullish/bearish/neutral 심리를 저장한다. Spring Boot가 API, 검증, 중복 제거, 저장, Swagger 조회를 담당하고 Python pipeline가 Playwright 기반 크롤링과 LLM 분석을 담당한다.
...
1. Python pipeline가 30분마다 community adapter를 실행한다.
2. adapter는 공개 HTTP 요청을 우선 사용하고, 필요하면 Playwright 렌더링 fallback을 사용한다.
3. pipeline이 종목 마스터 CSV의 티커/한글명/영문명/별칭으로 게시글 언급 종목을 매칭한다.
4. LLM provider가 종목별 sentiment와 confidence를 만든다.
5. pipeline이 `POST /internal/ingestions/community-posts`로 Spring에 결과를 전송한다.
