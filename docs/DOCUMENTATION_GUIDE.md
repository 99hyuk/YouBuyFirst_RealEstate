...
2. 도구 출력을 줄입니다. `Get-Content -TotalCount`, `Select-Object -First/-Skip`, `rg -n <키워드> <좁은 경로>`처럼 출력량을 먼저 제한합니다.
3. 로그는 집계합니다. 세션 JSONL, 브라우저 콘솔, DOM, Notion 페이지 전문은 대화에 붙이지 않고 오류명, 파일명, 수치만 남깁니다.
4. 검증은 유지합니다. 테스트, `git diff --check`, PR 라벨/본문 확인, 필요한 브라우저 QA는 생략하지 않고 결과만 짧게 보고합니다.
5. 수치로 확인합니다. 채팅 오류가 났거나 최적화 PR을 낸 뒤에는 이전/현재 세션의 `last input` 최대/평균을 비교합니다.
