...
4. 병렬 작업이면 `docs/workstreams/README.md`와 담당 트랙 문서를 읽습니다.
5. 필요한 경우 `docs/work-units/`에 작업 단위 문서를 만들거나 갱신합니다.
6. 관련 테스트를 실행합니다.
7. 작업 상태나 범위가 바뀌면 `docs/CURRENT_HANDOFF.md`와 `docs/TASKS.md`를 갱신합니다.
8. `git`과 `gh`로 직접 push, PR 생성, 라벨 지정, CI 확인을 수행합니다.
