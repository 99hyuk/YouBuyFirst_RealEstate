...
import type { RealEstatePricePoint } from '../fixtures/realestate-price-chart';

type ChartRange = '1M' | '3M' | '6M' | '1Y' | '3Y' | '5Y';
type PricePointMode = '일' | '주' | '월';
...
const pointMode = ref<PricePointMode>('일');
...
const pointModes = ['일', '주', '월'] as const;
...
const hoveredPricePoint = ref<RealEstatePricePoint | null>(null);
...
const modeDivisor = computed(() => {
  if (pointMode.value === '주') return 5;
  if (pointMode.value === '월') return 21;
  return 1;
});

const aggregatePricePoints = (pricePoints: RealEstatePricePoint[], mode: Exclude<PricePointMode, '일'>) => {
  const groups = new Map<string, RealEstatePricePoint[]>();

  pricePoints.forEach((pricePoint) => {
    const date = new Date(`${pricePoint.time}T00:00:00`);
    const key =
      mode === '월'
  ...
  return [...groups.values()].map((group) => ({
    ...
    trade: group.reduce((sum, item) => sum + item.trade, 0),
    rent: group.reduce((sum, item) => sum + item.rent, 0),
    supply: group.reduce((sum, item) => sum + item.supply, 0)
  }));
};

const chartPricePoints = computed(() => {
  if (pointMode.value === '일') return props.pricePoints;
  return aggregatePricePoints(props.pricePoints, pointMode.value);
});

const rangeSize = computed(() => {
  const days = observationRanges[selectedRange.value];
  if (!Number.isFinite(days)) return chartPricePoints.value.length;
  return Math.max(1, Math.ceil(days / modeDivisor.value));
});

const selectedLogicalWindow = computed(() => {
  const count = chartPricePoints.value.length;
...
const pricePointsInLogicalRange = (range: { from: number; to: number } | null) => {
  const pricePoints = chartPricePoints.value;
  if (!range || pricePoints.length === 0) return pricePoints;

  const from = Math.max(0, Math.floor(range.from));
  const to = Math.min(pricePoints.length - 1, Math.ceil(range.to));
  if (from > to) return [];

  return pricePoints.slice(from, to + 1);
};

const visibleRangePricePoints = computed(() => pricePointsInLogicalRange(visibleLogicalRange.value ?? selectedLogicalWindow.value));
const inspectedPricePoint = computed(() => hoveredPricePoint.value ?? visibleRangePricePoints.value.at(-1) ?? chartPricePoints.value.at(-1));
const latest = computed(() => inspectedPricePoint.value);
const previous = computed(() => {
  const index = chartPricePoints.value.findIndex((item) => item.time === inspectedPricePoint.value?.time);
  return chartPricePoints.value[index - 1] ?? visibleRangePricePoints.value.at(-2) ?? chartPricePoints.value.at(-2);
});
...
const formatPrice = (value: number) => `${value.toLocaleString('ko-KR', { maximumFractionDigits: 2 })}${props.priceUnit}`;
...
const formatAxisDate = (value?: string) => {
  ...
  return new Intl.DateTimeFormat('ko-KR', {
    year: pointMode.value === '월' ? 'numeric' : undefined,
    month: '2-digit',
    day: pointMode.value === '월' ? undefined : '2-digit'
  }).format(date);
};
...
const visibleAxisPricePoints = computed(() => {
  const pricePoints = visibleRangePricePoints.value;
  return pricePoints.length ? pricePoints : chartPricePoints.value;
});

const formatAxisTickLabel = (value: string, index: number, pricePoints: RealEstatePricePoint[]) => {
  const date = toChartDate(value);
  if (!date) return value;

  const visibleCount = pricePoints.length;
  if (pointMode.value === '월' || visibleCount >= 900) {
  ...
  if (pointMode.value === '주' || visibleCount >= 110) {
    return `${date.getMonth() + 1}월`;
  }

  const previous = toChartDate(pricePoints[index - 1]?.time);
...
const axisTickLabels = computed(() => {
  const pricePoints = visibleAxisPricePoints.value;
  if (!pricePoints.length) return [];

  const indexes = new Set<number>();
  if (pricePoints.length >= 900) {
    let currentYear: number | null = null;
    indexes.add(0);
    pricePoints.forEach((pricePoint, index) => {
      const year = toChartDate(pricePoint.time)?.getFullYear();
    ...
    const targetCount = Math.min(pricePoints.length, 9);
    for (let tickIndex = 0; tickIndex < targetCount; tickIndex += 1) {
      indexes.add(Math.round((tickIndex * (pricePoints.length - 1)) / (targetCount - 1)));
    }
  }
  ...
  const ticks = [...indexes]
    .sort((a, b) => a - b)
    .map((index) => ({
      key: `${pricePoints[index].time}-${index}`,
      label: formatAxisTickLabel(pricePoints[index].time, index, pricePoints)
    }));
...
const movingAverage = (source: RealEstatePricePoint[], windowSize: number) =>
  source
    .map((item, index) => {
      ...
      const value = window.reduce((sum, pricePoint) => sum + pricePoint.close, 0) / window.length;
...
const signalMarkers = computed<SeriesMarker<Time>[]>(() => {
  ...
  chartPricePoints.value.forEach((item, index) => {
...
const flowSummary = computed(() => {
  const rangePricePoints = visibleRangePricePoints.value.length ? visibleRangePricePoints.value : chartPricePoints.value;
  const totals = rangePricePoints.reduce(
    (acc, pricePoint) => {
      acc.trade += pricePoint.trade;
      acc.rent += pricePoint.rent;
      acc.supply += pricePoint.supply;
      return acc;
    },
    { trade: 0, rent: 0, supply: 0 }
  );

  return [
    { label: '매매', value: totals.trade },
    { label: '전월세', value: totals.rent },
    { label: '공급', value: totals.supply }
  ];
});
...
const flowBars = computed(() => {
  ...
  const marketActivity = flowSummary.value
    .filter((item) => item.label !== '공급')
    .reduce((sum, item) => sum + Math.max(item.value, 0), 0);
  const supplyPressure = Math.max(flowSummary.value.find((item) => item.label === '공급')?.value ?? 0, 0);
  const flowWindowSize = Math.max((visibleRangePricePoints.value.length ? visibleRangePricePoints.value : chartPricePoints.value).length, 1);
  const ma = Math.round((marketActivity - supplyPressure) / flowWindowSize);

  return [
    { label: '거래·전월세', value: marketActivity },
    { label: '공급 압력', value: -supplyPressure },
    { label: '평균', value: ma }
  ];
});
...
const setPricePointMode = (mode: (typeof pointModes)[number]) => {
  pointMode.value = mode;
  hoveredPricePoint.value = null;
};
...
const renderChart = async () => {
  ...
  if (isTestMode || !chartEl.value || chartPricePoints.value.length === 0) {
    return;
  }
  ...
  priceSeries.setData(
    chartPricePoints.value.map((item) => ({
      time: item.time as Time,
      value: item.close
    }))
  );
  ...
  if (showSignals.value) {
    createSeriesMarkers(priceSeries, signalMarkers.value);
  }
  ...
  movingAverageLines.value.forEach((line) => {
    ...
    series?.setData(movingAverage(chartPricePoints.value, line.window));
  });
  ...
  chart.subscribeCrosshairMove((param) => {
    ...
    hoveredPricePoint.value = hoverTime ? chartPricePoints.value.find((item) => item.time === timeToDateString(hoverTime)) ?? null : null;
  });
...
<template>
  <div class="realestate-price-chart-shell">
    <div class="realestate-chart-toolbar">
      <div>
        <strong>{{ title }}</strong>
    ...
    <div class="chart-feature-strip" aria-label="차트 기능">
      <div class="chart-feature-group">
        <span class="chart-feature-label">단위</span>
        <div class="chart-feature-controls">
          <button
            v-for="mode in pointModes"
            :key="mode"
            type="button"
            :class="{ active: pointMode === mode }"
            @click="setPricePointMode(mode)"
          >
            {{ mode }}
          </button>
        </div>
      ...
      <div class="chart-feature-group wide">
        ...
        <div class="chart-feature-controls">
          <button type="button" :class="{ active: showSignals }" @click="showSignals = !showSignals">관찰</button>
    ...
    <div v-if="showSignals" class="chart-signal-legend" aria-label="관찰 포인트 설명">
      <span><b>관심</b> 가격 하단권 + 거래·전월세 유입</span>
      <span><b>주의</b> 가격 상단권 + 공급 부담 확대</span>
      <em>참고용 관찰 표시 · 가격 예측이나 행동 권유가 아닙니다</em>
    </div>
    ...
    <div class="chart-price-stat-strip" v-if="latest">
      ...
      <span>최근 {{ formatPrice(latest.close) }}</span>
      <strong :class="change.tone">{{ changeSummary }}</strong>
    </div>

    <div ref="chartEl" class="realestate-lightweight-chart">
      <div v-if="isTestMode || !chartRendered" class="realestate-chart-test-fallback">
        <strong>{{ providerTargetId }}</strong>
        <span>{{ title }} · {{ chartPricePoints.length }}개 가격 포인트</span>
      </div>
    </div>
    ...
    <div class="realestate-chart-lower-grid" :class="{ 'volume-only': !showFlowSummary }">
      <article class="volume-brief">
        <span>거래량</span>
        <strong>{{ latest ? formatCompact(latest.volume) : '-' }}{{ volumeUnit }}</strong>
        <em>가격 포인트 하단 히스토그램 표시</em>
      </article>
      <article v-if="showFlowSummary" class="flow-brief">
        <div>
          ...
          <div class="flow-mode-tabs" aria-label="매매동향 보기 방식">
            ...
            <button type="button" :class="{ active: flowView === 'subjects' }" @click="flowView = 'subjects'">출처별</button>
          </div>
